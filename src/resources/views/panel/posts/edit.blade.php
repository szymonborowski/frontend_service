...
@section('panel-content')
    <form method="POST" action="{{ route('panel.posts.update', $post['id']) }}" class="space-y-6">
        ...
        <div>
            <label for="slug" class="block text-sm font-medium text-gray-700 mb-2">{{ __('posts.slug') }}</label>
            <input type="text" name="slug" id="slug"
                   class="appearance-none rounded relative block w-full px-3 py-2 border @error('slug') border-red-500 @else border-gray-300 @enderror placeholder-gray-500 text-gray-900 focus:outline-none focus:ring-sky-700 focus:border-sky-700 sm:text-sm font-mono"
                   value="{{ old('slug', $post['slug'] ?? '') }}"
                   placeholder="my-post-url">
            <p class="mt-1 text-xs text-gray-500">{{ __('posts.slug_hint') }}</p>
            @error('slug')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div>
            ...
            <textarea name="content" id="content" rows="15" required
                      ...
                      placeholder="{{ __('posts.content_placeholder') }}">{{ old('content', $post['content'] ?? '') }}</textarea>
            @error('content')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
        </div>
