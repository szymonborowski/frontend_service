            ...
            <main class="lg:col-span-2">
                <div class="bg-white rounded-lg shadow p-6">
                    ...
                    @if(($category['posts_count'] ?? 0) > 0)
                        <p class="text-sm text-gray-500 mb-6">{{ __('general.posts_count', ['count' => $category['posts_count']]) }}</p>
                    @endif
                    <ul class="space-y-6">
                                    ...
                                    <div class="flex items-center space-x-2 mb-2">
                                        @if($post['categories'] ?? [])
                                            @foreach($post['categories'] as $cat)
                                                <a href="{{ route('category.show', $cat['slug']) }}" class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-sky-100 text-sky-800 hover:bg-sky-200">
                                                    {{ $cat['name'] }}
                                                </a>
                                            @endforeach
                                        @endif
                                    ...
                                    @if($post['tags'] ?? [])
                                        <div class="mt-2 flex flex-wrap gap-1">
                                            @foreach($post['tags'] as $tag)
                                                <a href="{{ route('tag.show', $tag['slug']) }}" class="text-xs text-gray-500 hover:text-sky-800">#{{ $tag['name'] }}</a>
                                            @endforeach
                                        </div>
                                    @endif
                                </article>
            ...
            <aside class="lg:col-span-1 space-y-6">
                <div class="bg-white rounded-lg shadow p-6">
                    ...
                    <ul class="space-y-2">
                        @forelse($categories as $cat)
                            <li>
                                <a href="{{ route('category.show', $cat['slug']) }}" class="flex items-center justify-between text-sm text-gray-600 hover:text-sky-800 {{ ($cat['slug'] ?? '') === ($category['slug'] ?? '') ? 'font-medium text-sky-800' : '' }}">
                                    <span>{{ $cat['name'] }}</span>
                ...
                <div class="bg-white rounded-lg shadow p-6">
                    ...
                    <div class="flex flex-wrap gap-2">
                        @forelse($tags as $t)
                            <a href="{{ route('tag.show', $t['slug']) }}" class="inline-flex items-center px-2 py-1 rounded text-xs font-medium bg-gray-100 text-gray-600 hover:bg-sky-100 hover:text-sky-800">
                                #{{ $t['name'] }}
                            </a>
                        @empty
