...
<div {{ $attributes->merge(['class' => 'bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6 flex flex-col justify-between']) }}>
    <div>
        <div class="flex items-center gap-2 mb-4">
            ...
            <h2 class="text-lg font-semibold text-gray-900 dark:text-gray-100">{{ __('general.recent_commits') }}</h2>
        </div>
                ...
                @foreach($commits as $commit)
                    <li class="flex items-start gap-3">
                        <a href="{{ $commit['url'] }}"
                           target="_blank"
                           rel="noopener"
                           class="font-mono text-xs bg-gray-100 dark:bg-gray-700 text-sky-700 dark:text-sky-400 px-1.5 py-0.5 rounded shrink-0 hover:underline mt-0.5">
                            {{ $commit['short'] }}
                        </a>
